...
max_length = st.sidebar.slider("Output Length (tokens)", min_value=100, max_value=2000, value=300, step=50)
...
selected_model = st.sidebar.radio("Select Model", options=available_models, index=0)

# --- User Input ---
st.header("Generate a Story")
user_query = st.text_input("Enter your story topic")
...
@st.cache_data
def load_text_data():
    df = pd.read_csv(r"C:\Users\dhwan\Desktop\clg\LLM\CIA\1k_stories_100_genre.csv")
...
if st.button("Get Stories"):
    with st.spinner("Retrieving similar stories..."):
        ...
        st.subheader("Similar Existing Stories")
